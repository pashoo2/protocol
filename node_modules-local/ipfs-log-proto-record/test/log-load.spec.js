  ...
  describe('Log - Load (' + IPFS + ')', function () {
    ...
    describe('fromEntry', () => {
      ...
      it('sorts', async () => {
        ...
        const values = await log.values()
        let fetchOrder = values.slice().sort(Entry.compare)
        assert.deepStrictEqual(fetchOrder.map(e => e.payload), expectedData)

        let reverseOrder = values.slice().reverse().sort(Entry.compare)
        assert.deepStrictEqual(fetchOrder, reverseOrder)

        let hashOrder = values.slice().sort((a, b) => a.hash > b.hash).sort(Entry.compare)
        assert.deepStrictEqual(fetchOrder, hashOrder)

        let randomOrder2 = values.slice().sort((a, b) => 0.5 - Math.random()).sort(Entry.compare)
        assert.deepStrictEqual(fetchOrder, randomOrder2)

        // partial data
        let partialLog = values.filter(e => e.payload !== 'entryC0').sort(Entry.compare)
        assert.deepStrictEqual(partialLog.map(e => e.payload), expectedData2)

        let partialLog2 = values.filter(e => e.payload !== 'entryA10').sort(Entry.compare)
        assert.deepStrictEqual(partialLog2.map(e => e.payload), expectedData3)

        let partialLog3 = values.filter(e => e.payload !== 'entryB5').sort(Entry.compare)
      ...
      it('throws an error if the tiebreaker returns zero', async () => {
        ...
        await firstWriteWinsLog.join(testLog.log)
        const firstWriteWinsLogValues = await firstWriteWinsLog.values()
        assert.throws(() => firstWriteWinsLogValues, Error, 'Error Thrown')
      })
