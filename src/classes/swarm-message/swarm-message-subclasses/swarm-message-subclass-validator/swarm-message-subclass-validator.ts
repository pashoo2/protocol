...
import { commonUtilsArrayDeleteFromArray, commonUtilsArrayDoCallbackTillNoError, commonUtilsArrayCalculateLengthOfIntegerArray } from 'utils/common-utils/common-utils';
import { TType, IMessageValidatorOptions, TPayload } from './swarm-message-subclass-validator.types';
import { SWARM_MESSAGE_SUBCLASS_VALIDATOR_PAYLOAD_MAX_LENGTH_BYTES, SWARM_MESSAGE_SUBCLASS_VALIDATOR_PAYLOAD_MIN_LENGTH_BYTES } from './swarm-message-subclass-validator.const';
...
export class SwarmMessageSubclassValidator {
    ...
    public validateUserIdentifier(
        ...
        if (!(userId instanceof CentralAuthorityIdentity) && userId !== 'string') {
            return new Error('User id must be a string or an instance of the CentralAuthorityIdentity');
        }
    ...
    public validatePayload(pld: TPayload): boolean | Error {
        ...
        if (pld instanceof Array) {
            len = commonUtilsArrayCalculateLengthOfIntegerArray(
                pld,
                255,
                0,
            );
            ...
            if (len instanceof Error) {
                console.error(len);
                return new Error('The value of the payload is not a valid array with byte-length integers');
            }
            ...
            return new Error('The payload value must be a string, an instance of a byte-integers Array, Uint8Array, ArrayBuffer or SharedArrayBuffer');
        }
        if (typeof len !== 'number') {
            return new Error('Unknown error has occurred while calculating the lenght of the payload');
        }
        ...
        if (options) {
            ...
            if (payloadMaxLength) {
                if (typeof payloadMaxLength === 'number') {
                   ...
                   throw new Error('The value of the "payloadMaxLength" option must be a number');
                }
            }
            if (payloadMinLength) {
                if (typeof payloadMinLength === 'number') {
                   ...
                   throw new Error('The value of the "payloadMinLength" option must be a number');
                }
            }
            if (issuersList) {
                if (issuersList instanceof Array) {
                    const setIssuersListResult = commonUtilsArrayDoCallbackTillNoError<string>(issuersList, this.addIssuer);

                    if (setIssuersListResult instanceof Error) {
                        throw setIssuersListResult;
                    }
                } else {
                    throw new Error('The value of the "issuersList" option must be an Array');
                }
            }
            if (typesList) {
                if (typesList instanceof Array) {
                    const setTypesListResult = commonUtilsArrayDoCallbackTillNoError<TType>(typesList, this.addType);

                    if (setTypesListResult instanceof Error) {
                        throw setTypesListResult;
                    }
                } else {
                    throw new Error('The value of the "typesList" option must be an Array');
                }
            }
        }
    }
