...
export const getCryptoKeyByLogin = (
  login: string
): Promise<CryptoKey | Error> => {
  return generatePasswordKeyByPasswordString(
    login,
    SECRET_STORAGE_CLASS_UTILS_LOGIN_SALT
  );
};

export const encryptValueByLogin = async (
  login: string,
  value: string
...
export const decryptValueByLogin = async (
  login: string,
  value: string
