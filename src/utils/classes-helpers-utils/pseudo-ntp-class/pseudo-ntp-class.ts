...
import { PSEUDO_NTP_CLASS_DEFAULT_OPTIONS, PSEUDO_NTP_CLASS_LOGS_PREFIX, PSEUDO_NTP_CLASS_EVENTS } from './pseudo-ntp-class.const';
...
export class PseudoNTPClass extends EventEmitter<IPseudoNTPClassEvents> {
    ...
    protected log = (message: Error | string) => {
        ...
        const consoleMethod = message instanceof Error
            ? 'error'
            : 'log';
    ...
    protected setOptions(options: IPseudoNTPClassOptions) {
        ...
        if (!(serversPool instanceof Array)
            || !serversPool.length
        ) {
    ...
    protected convertServerResponseRaw(responseRaw: TPseudoNTPClassResponseRaw): TPseudoNTPClassServerResponse | Error {
        return '';
    }
